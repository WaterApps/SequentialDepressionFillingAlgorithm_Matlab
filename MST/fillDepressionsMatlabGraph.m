function[dem, flow_direction, pits, depthFlow, rainfall_excess, runoff] = ...
    fillDepressionsMatlabGraph(fillRainfallExcess, dem, flow_direction, pits, pairs, cellIndexes, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, R, visualize_merging, graph)
...
%         [val, ord] = min(max(dem(pairs{second_pit}(:, 1:2)), [], 2));
        ...
        %NEW SPILLOVERELEV
        [a,~] = outedges(graph, second_pit)
        [m, n] = min(graph.Edges.Weights(a));
        spilloverElevation(second_pit) = m;
        cells = [graph.Edges.fromCellIndex(a(n)), graph.Edges.toCellIndex(a(n))];
        cellOverflowInto(second_pit) = cells(pits(cells) ~= second_pit);
...
%      end
    ...
    [m, n] = min(graph.Edges.Weights(a));
    second_pit = pits(graph.Edges.toCellIndex(a(n)));
