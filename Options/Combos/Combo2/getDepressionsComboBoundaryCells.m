function[pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, boundaryCells] = getDepressionsComboBoundaryCells(dem, flow_direction, cellsize)
...
boundaryCells = cell(length(pitId), 1);
...
for p = 1 : length(pitCell)
    ...
    boundaryCells{p} = zeros(areaCellCount(p), 1);
    [r, c] = ind2sub(size(dem), pitCell(p));
    indicesToCheck(1, :) = [r, c];
    j = 1;
    k = 1;
    spilloverElevation(p) = realmax('double');
    for i = 1 :  areaCellCount(p) % walk through indicesToCheck
        incrementK = false;
    ...
    boundaryCells{p}(k:end) = []; % remove extra preallocated space   
    indicesToCheck(j:end, :) = [];
    % Accumulate those elevations that are less than spillover
    
    %volume(p) = sum((spilloverElevation(p) - dem(dem < spilloverElevation(p) & pits == p))*cellsize*cellsize);
    indicesToCheck = sub2ind(size(dem), indicesToCheck(:, 1), indicesToCheck(:, 2))';
    lessThans = indicesToCheck(dem(indicesToCheck) <= spilloverElevation(p));
