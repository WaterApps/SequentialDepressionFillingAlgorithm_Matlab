function[pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, lineSegments, elevations, cellIndexes] = getDepressionsDownstreamTraversal(dem, flow_direction, cellsize)
...
pits = int32(zeros(size(dem), 'single')); % matrix identifying pits
...
for p = 1 : numel(pits)
    ...
    curPitId = pits(p);
    if (curPitId > 0)
        if (mod(j(curPitId), chunksize) == 1) % preallocate more space
            cellIndexes{curPitId}(j(curPitId)+chunksize) =  0;
        end
        areaCellCount(curPitId) = areaCellCount(curPitId) + 1;
        cellIndexes{curPitId}(j(curPitId)) = p;
        j(curPitId) = j(curPitId) + 1;
    end
end

for p = 1 : pit_count
    cellIndexes{p}(j(p):end) = [];
end

function child = recursiveMarkPits(i)
    fd = flow_direction(i);
    child = NaN;
    if (fd == -2) % edge cells marked as -2
        return
    elseif (fd == 0) % points to no cell
        return
    elseif (fd == -1) % depression due to topography
        child = pits(i);
    elseif (pits(fd) == 0) % downstream is yet unidentified in the pits matrix
        child = recursiveMarkPits(fd);
        pits(i) = child;
    else % downstream already been set; count yourself otw out.
        child = pits(fd);
        pits(i) = child;
    end
    return
...
elevations = cell(pit_count, 1);
lineSegments = cell(pit_count, 1);
[numrows, numcols] = size(dem);
parfor p = 1 : length(cellIndexes)
    elevations{p} = ones(length(cellIndexes{p}), 2);
    elevations{p}(:, 1) = dem(cellIndexes{p})'; 
    ...
    % Use array of elevations to compute volume; prefill up to spillover
    an = any(elevations{p}, 2); 
    elevations{p} = elevations{p}(an, :); % trim zero rows 
    [uniq, ~, ic] = unique(elevations{p}(:,1)); % find unique values 
    elevations{p} = [uniq, histc(elevations{p}(:,1), uniq)]; % remove duplicates 
    
    % get cell indexes associated with each unique value
    newElevations = cell(length(uniq), 1); 
    for m = 1 : length(ic)
        newElevations{ic(m)} = [newElevations{ic(m)}, cellIndexes{p}(m)]; 
    end 
    cellIndexes{p} = newElevations; 
    newElevations = []; %deallocate 
    
    % Accumulate those elevations that are less than spillover
    lessThans = elevations{p}(:, 1) <= spilloverElevation(p); 
    volume(p) = sum((spilloverElevation(p) - elevations{p}(lessThans, 1)).*elevations{p}(lessThans, 2)*cellsize*cellsize); 
    count = sum(elevations{p}(lessThans, 2));
    elevations{p}(lessThans, :) = 0; 
    elevations{p}(1, :) = [spilloverElevation(p), count];
    cellIndexes{p}(1) = {[cellIndexes{p}{lessThans}]};
    
    an = any(elevations{p}, 2); % trim zero rows 
    elevations{p} = elevations{p}(an, :);  
    cellIndexes{p} = cellIndexes{p}(an, :);
    lessThans = []; 
