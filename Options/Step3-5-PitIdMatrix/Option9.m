function[pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, lineSegments, cellIndexes] = getDepressionsParents(dem, flow_direction, flow_direction_parents, cellsize)
...
for p = 1 : length(pitCell)
    ...
    areaCellCount(p) = length(j);
end
% for p = 1 : length(cellIndexes)
%     pits(cellIndexes{p}) = pits(p);
% end
