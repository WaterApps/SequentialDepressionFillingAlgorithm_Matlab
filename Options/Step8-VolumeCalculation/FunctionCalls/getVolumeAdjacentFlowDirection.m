function[volume] = getVolumeAdjacentFlowDirection(areaCellCount, pitCell, spilloverElevation, dem, flow_direction, cellsize)
    indexes = reshape(1: numel(flow_direction), size(flow_direction));
    indicesToCheck = zeros(areaCellCount, 2);
    [r, c] = ind2sub(size(flow_direction), pitCell);
    indicesToCheck(1, :) = [r, c];
    j = 1;
    i = 1;
    volume = double(0);
    [numrows, numcols] = size(dem);
    while i <= j
        r = indicesToCheck(i, 1);
        c = indicesToCheck(i, 2);
        i = i + 1;
        volume = volume + ((spilloverElevation - dem(r,c))*cellsize*cellsize);
        for x = -1 : 1 % loop through neighboring cells
            for y = -1 : 1
                if x == 0 && y ==0
                    continue; % skip center cell of 3x3 neighborhood
                end
                if r+y > numrows || r+y < 1 || c+x > numcols || c+x < 1
                    continue; % skip neighbors outside the dem range
                end

                curCell = indexes(r, c);
                if flow_direction(r+y, c+x) == curCell && dem(r+y, c+x) <= spilloverElevation
                    j = j + 1;
                    indicesToCheck(j, :) = [r+y, c+x];
                end
            end
        end
    end
end