...
while ~isnan(vca(second_pit)) && ~isinf(vca(first_pit))
    ...
    lineSegments{second_pit} = [lineSegments{first_pit}; lineSegments{second_pit}];
    lineSegments{second_pit} = lineSegments{second_pit}(any(pits(lineSegments{second_pit}) ~= second_pit, 2), :);

    % get minimum spillover elevation and cellOverflowInto
    [val, ord] = min(max(dem(lineSegments{second_pit}(:, 1:2)), [], 2));
    spilloverElevation(second_pit) = val;
    cellOverflowInto(second_pit) = lineSegments{second_pit}(ord, 3);
    ...
    volume(second_pit) = filledVolume(second_pit) + sum(sum(dem(pits == second_pit & dem < spilloverElevation(second_pit))));
