...
if (false)%exist(flowDirectionFile, 'file') == 2)
    ...
    flowtic = tic;
    [flow_direction] = d8FlowDirectionOriginal(dem);
    flowtoc = toc(flowtic);
...
pitsFile = strcat('ComboOriginalCedarUpper_', dem_options(dem_selection, :), '_Pits.mat');
if (false)%exist(pitsFile, 'file') == 2)
    disp('Pits data was found')
    load(pitsFile);
else
    disp('Creating Pit Database');
    [pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto] = getDepressionsComboOriginal(dem, flow_direction, georef_info.CellExtentInWorldX);
    save(pitsFile, 'pits', 'pitId', 'pitCell', 'areaCellCount', 'spilloverElevation', 'vca', 'volume', 'filledVolume', 'cellOverflowInto')
end
%% Fill Pits
disp('Filling Pits')
fillFile = strcat('ComboOriginalCedarUpper_', dem_options(dem_selection, :), '_outputs.mat');
if (false) %exist(fillFile, 'file') == 2)
    ...
    [times, fill_dem, fill_flow_direction, fill_pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow, first_pit_areas, second_pit_areas] = ...
        fillDepressionsComboOriginal(dem, flow_direction, pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarUpperOutletPoint, georef_info, dem_options(dem_selection, :));
    filltoc = toc(filltic);
%    save(fillFile);
end
...
profile off;
%profsave(profile('info'),strcat('ComboOriginalCedarUpper_', dem_options(dem_selection, :), '_profile'));
profile viewer;
catch ME
    profile off
    rethrow(ME)
