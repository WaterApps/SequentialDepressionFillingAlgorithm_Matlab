...
flowDirectionFile = strcat('ComboBoundaryCedarUpper_', dem_options(dem_selection, :), '_FlowDirection.mat');
if (false) %flowDirectionFile, 'file') == 2)
...
% [flow_direction_parents] = d8FlowDirectionParentsSparse(flow_direction);

%% Identify Pits, Compute Matrix/Map with Pit ID for each cell
pitsFile = strcat('ComboBoundaryCedarUpper_', dem_options(dem_selection, :), '_Pits.mat');
if (false)%exist(pitsFile, 'file') == 2)
    disp('Pits data was found')
    load(pitsFile);
else
    disp('Creating Pit Database');
    [pits, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, boundaryCells] = getDepressionsComboBoundaryCells(dem, flow_direction, georef_info.CellExtentInWorldX);
    save(pitsFile, 'pits', 'pitId', 'pitCell', 'areaCellCount', 'spilloverElevation', 'vca', 'volume', 'filledVolume', 'cellOverflowInto', 'boundaryCells')
end

%% Fill Pits
disp('Filling Pits')
fillFile = strcat('ComboBoundaryCedarUpper_', dem_options(dem_selection, :), '_outputs.mat');
if (false) %exist(fillFile, 'file') == 2)
    ...
    filltic = tic;
    [times, fill_dem, fill_flow_direction, fill_pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow] = ...
        fillDepressionsComboBoundaryCells(dem, flow_direction, pits, boundaryCells, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarUpperOutletPoint, georef_info, dem_options(dem_selection, :));
    filltoc = toc(filltic);
    save(fillFile);
end
...
profile off;
%profsave(profile('info'),strcat('ComboBoundaryCedarUpper_', dem_options(dem_selection, :), '_profile'));
profile viewer;
catch ME
    profile off
    rethrow(ME)
