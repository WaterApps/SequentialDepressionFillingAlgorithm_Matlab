function[dem, flow_direction, pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow, first_pit_areas, second_pit_areas] = ...
    fillDepressionsComboCacheSortAll(dem, flow_direction, pits, pairs, cellIndexes, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarCreekOutletPoint, R, dem_selection)
...
cellIndexes = cellIndexes(order);
pairs = pairs(order);

nonNanCount = nansum(nansum(~isnan(dem)));
disp(strcat('Cells with defined values: ', num2str(nonNanCount)));

% Find the total number of iterations/merges; (+1 for the initial state)
potential_merges = length(pitCell);
% Preallocate arrays of variables that will be used in analysis
first_pit_areas = zeros(potential_merges, 1);% cells
second_pit_areas = zeros(potential_merges, 1);% cells
...
first_pit = 1;
second_pit_id = pits(cellOverflowInto(1));
...
while idx <= potential_merges
    ...
    cellOverflowInto(second_pit) = pairs{second_pit}(ord, pits(pairs{second_pit}(ord, :)) ~= second_pit_id);
    ...
    idx = idx + 1;
%     first_pit = order(1);
    second_pit_id = pits(cellOverflowInto(first_pit));
    second_pit = find(pitId == second_pit_id);
end
