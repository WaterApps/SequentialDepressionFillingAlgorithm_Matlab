function[dem, flow_direction, pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow, first_pit_areas, second_pit_areas] = ...
    fillDepressionsSortInsert(dem, flow_direction, pits, pairs, cellIndexes, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarCreekOutletPoint, R, dem_selection)
...
while idx <= potential_merges
    ...
    oldIdx = find(newOrder == second_pit);
    vcaIdx = find(vca(newOrder) >= vca(second_pit), 1); % find where to insert second_pit
    if oldIdx < vcaIdx
        newOrder = [newOrder(2:oldIdx-1); newOrder(oldIdx+1:vcaIdx-1); second_pit; newOrder(vcaIdx:end); first_pit]; % insert; move second_pit from the end to its new position
    else
        newOrder = [newOrder(2:vcaIdx-1); second_pit; newOrder(vcaIdx:oldIdx-1); newOrder(oldIdx+1:end); first_pit]; % insert; move second_pit from the end to its new position
    end
    idx = idx + 1;
    first_pit = newOrder(1);
    second_pit = pits(cellOverflowInto(first_pit));
