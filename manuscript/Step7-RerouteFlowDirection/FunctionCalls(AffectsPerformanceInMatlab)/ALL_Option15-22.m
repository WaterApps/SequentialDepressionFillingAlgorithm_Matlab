function[dem, flow_direction, pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow] = fillDepressionsReRouteOptions2(dem, flow_direction, pits, pairs, boundaryCells, cellIndexes, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarCreekOutletPoint, R, dem_selection)
...
first_pit = order(1);
second_pit = pits(cellOverflowInto(first_pit));ii = double(flow_direction(~isnan(flow_direction) & flow_direction > 0));
jj = find(~isnan(flow_direction) & flow_direction > 0);
ii = double(flow_direction(~isnan(flow_direction) & flow_direction > 0));
jj = find(~isnan(flow_direction) & flow_direction > 0);
fdpsparse1 = sparse(ii, jj, true, numel(flow_direction), numel(flow_direction));
fdpsparse2 = sparse(ii, jj, true, numel(flow_direction), numel(flow_direction), length(jj));
...
%% updateFlowDirectionNonAdjacent
    copyFd = flow_direction;
    flow_direction(pitCell) = cellOverflowInto;
    flow_direction = copyFd;
    
%% updateFlowDirectionParentsNonAdjacent
    copyFdp = flow_direction_parents;
    flow_direction_parents{cellOverflowInto} = [flow_direction_parents{cellOverflowInto}, pitCell];
    flow_direction_parents = copyFdp;
%% updateFlowDirectionParentsSparse
    fdpsparse1(cellOverflowInto(first_pit), pitCell(first_pit)) = true;
    flow_direction_parents = copyFdp;
%% updateFlowDirectionParentsSparsePreall
    fdpsparse2(cellOverflowInto(first_pit), pitCell(first_pit)) = true;
%% updateFlowDirectionAdjacent
