function[dem, flow_direction, pits, rainfall_excess, runoff_areas, number_of_pits, mean_depth, std_depth, mean_area, std_area, storage_volume, runoff_volume, depthFlow] = fillDepressionsUpdatePitsDem2(dem, flow_direction, pits, pairs, cellIndexes, pitId, pitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, CedarCreekOutletPoint, R, dem_selection)
...
%% updatePitsDemParents
    indicesToCheck = zeros(areaCellCount(first_pit), 1);
    indicesToCheck(1) = pitCell(first_pit);
    j = 1;
    i = 1;
    while i <= j
        curCell = indicesToCheck(i);
        pits(curCell) = second_pit;
        if dem(curCell) <= spilloverElevation(first_pit)
            dem(curCell) = spilloverElevation(first_pit);
        end
        parents = flow_direction_parents{curCell};
        k = length(parents);
        indicesToCheck(j+1:j+k) = parents;
        j = j + k;
        i = i + 1;
    end
%%    
