function[volume] = getVolumeNaiveAdjacentNeighborhood(areaCellCount, pitCell, spilloverElevation, dem, pits, pitId, cellsize)
    temp = false(size(dem));
    ...
    while i <= j
        ...
        for x = -1 : 1 % loop through neighboring cells
            for y = -1 : 1
                ...
                if dem(r+y, c+x) <= spilloverElevation && pits(r+y, c+x) == pitId && ~temp(r+y, c+x)
                    temp(r+y, c+x) = true;
                    j = j + 1;
