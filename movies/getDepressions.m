function[pits, pairs, cellIndexes, pitId, pitCell, edgePitCell, areaCellCount, spilloverElevation, vca, volume, filledVolume, cellOverflowInto, perimeterCell] = getDepressions(dem, flow_direction, flow_direction_parents, cellsize)
...
edge_pit_count = sum(nansum(flow_direction == -2));
pitId = int32(1 : pit_count); % id of each pit
edgePitId = int32(-1 : -1 : (-1*edge_pit_count));
...
perimeterCell = int32(zeros(pit_count, 1)); %cell index of overflow location
...
pits(edgePitCell) = edgePitId; % Mark the pit bottom in the pits image. This is 
